...
$spam_words_file = "spam_words.txt"; // New file for spam words
$rate_limit_file = "rate_limit.txt"; // New file for rate limits
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['ban_user'])) {
        ...
        setBannedUserBalanceToZero($_POST['ban_user']); // Set balance to 0 when banning
    }
    ...
    if (isset($_POST['send_notice'])) {
        ...
        if (!empty($notice)) {
            $notice_entry = [
                'id' => uniqid(), // Add unique ID for each notice
                'time' => time(), // Store as timestamp for expiration check
                'content' => $notice,
    ...
    if (isset($_POST['send_bonus'])) {
        ...
        if ($amount > 0 && !empty($reason)) {
            ...
            $bonus_entry = [
                ...
                'duration' => $duration > 0 ? $duration * 3600 : 86400, // Default 24 hours
                'reason' => $reason,
                'claimed_by' => []
            ];
    ...
    <div class="admin-container">
        ...
        <div class="main-content">
            ...
            switch ($page) {
                ...
                case 'api':
                    $sms = @json_decode(@file_get_contents("api/sms.json"), true) ?? [];
                    $otp = @json_decode(@file_get_contents("api/otp.json"), true) ?? [];
                    ?>
                    <div class="card">
                        <div class="card-header">
                            ...
                            <form method="POST">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="sms_api_url" name="sms_api_url" class="form-control" 
                                           value="<?php echo htmlspecialchars($sms['api_url'] ?? ''); ?>" 
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="sms_status" name="sms_status" class="form-control">
                                        <option value="on" <?php echo ($sms['status'] ?? '') === 'on' ? 'selected' : ''; ?>>Enabled</option>
                                        <option value="off" <?php echo ($sms['status'] ?? '') === 'off' ? 'selected' : ''; ?>>Disabled</option>
                                    </select>
                                </div>
                            ...
                            <form method="POST">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="otp_api_url" name="otp_api_url" class="form-control" 
                                           value="<?php echo htmlspecialchars($otp['api_url'] ?? ''); ?>" 
                                           placeholder="http://example.com/api?number={number}&msg={msg}">
                                    <small style="color: #666;">Note: Use {number} and {msg} as placeholders</small>
                                </div>
                                <div class="form-group">
                                    ...
                                    <select id="otp_status" name="otp_status" class="form-control">
                                        <option value="on" <?php echo ($otp['status'] ?? '') === 'on' ? 'selected' : ''; ?>>Enabled</option>
                                        <option value="off" <?php echo ($otp['status'] ?? '') === 'off' ? 'selected' : ''; ?>>Disabled</option>
                                    </select>
                                </div>
                                <div style="display: flex; gap: 1rem;">
