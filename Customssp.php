...
if (!empty($aspNetCoreCookies)) {
    ...
    // 4. First, get the SMS page to get a fresh token (optional)
    ...
    // 6. Process response - FIXED HERE
    if ($smsHttpCode === 200) {
        // Clean the response
        $cleanResponse = trim($smsResponse, '"');
        
        // Check if response indicates success
        if (strtolower($cleanResponse) === 'success' || 
            strpos($smsResponse, 'Success') !== false ||
            strpos($smsResponse, 'success') !== false) {
            echo json_encode([
                'code' => 200,
                'status' => true,
            ...
            echo json_encode([
                'code' => 500,
                'status' => false,
                'message' => 'SMS sending failed. Unexpected response.',
                'debug' => [
                    'response' => $smsResponse,
                    'clean_response' => $cleanResponse,
                    'http_code' => $smsHttpCode
                ]
            ], JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
        }
    } else {
